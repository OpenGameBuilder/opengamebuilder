...
[[ "$environment" == staging || "$environment" == production ]] || {
  echo 'Unknown edge environment.' >&2
  exit 1
}
[[ "$base_url" =~ ^https://([a-zA-Z0-9.-]+)/?$ ]] || {
  echo 'Expected an HTTPS origin without a port or path.' >&2
  exit 1
}
...
response="$(curl --fail --show-error --silent \
  ...
  "${base_url%/}/health")"
[[ "$response" == "ok $environment" ]] || {
  echo "Unexpected ${environment} edge health response." >&2
  exit 1
}
