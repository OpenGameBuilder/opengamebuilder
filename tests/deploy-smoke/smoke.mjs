...
assert.match(
  baseUrl ?? "",
  /^https:\/\//,
  "A public HTTPS smoke URL is required",
);
assert.match(
  expectedRevision ?? "",
  /^[0-9a-f]{40}$/,
  "Expected source revision is required",
);
assert.match(
  expectedReleaseId ?? "",
  /^[a-zA-Z0-9][a-zA-Z0-9.-]{0,100}$/,
  "Expected release ID is required",
);
  ...
  for (let attempt = 1; attempt <= 6; attempt++) {
    ...
    try {
      ...
      assert.equal(
        ...
        200,
        "The page must successfully call /api/about",
      );
      const about = await aboutResponse.json();
      assert.equal(
        about.sourceRevision,
        expectedRevision,
        "The running API must match the requested source revision",
      );
      assert.equal(
        await page.locator("base").getAttribute("href"),
        `/releases/${expectedReleaseId}/`,
      );
      const title = `${about.applicationName} ${about.version}`;
      await page.waitForFunction(
        (expected) =>
          document.querySelector("#app h1")?.textContent?.includes(expected),
        title,
        { timeout: 30000 },
      );
      assert.deepEqual(
        pageErrors,
        [],
        "The frontend must start without page errors",
      );
      console.log(
        `PASS ${expectedReleaseId}: frontend rendered ${title} using API ${expectedRevision}`,
      );
      process.exitCode = 0;
      lastError = undefined;
      break;
