...
cleanup() {
    case "${test_root}" in
        ...
        *) echo "Refusing to remove unexpected test directory: ${test_root}" >&2; exit 1 ;;
    esac
}
trap cleanup EXIT
...
assert_status() { [ "${status}" = "$1" ] || { cat "$output" >&2; fail "Expected exit $1, got ${status}"; }; }
assert_contains() { grep -Fq -- "$1" "$2" || { cat "$2" >&2; fail "Missing '$1' in $2"; }; }
assert_not_contains() { if grep -Fq -- "$1" "$2"; then cat "$2" >&2; fail "Unexpected '$1' in $2"; fi; }
...
MOCK_RELEASE_LIST_FAIL=1; export MOCK_RELEASE_LIST_FAIL
validate main
...
MOCK_PR_NUMBER=42; export MOCK_PR_NUMBER
post_standard 1.9.0 1.10.0
assert_status 0
...
MOCK_PR_LIST_FAIL=1; export MOCK_PR_LIST_FAIL
post_standard 1.9.0 1.10.0
...
MOCK_PR_CREATE_FAIL=1; export MOCK_PR_CREATE_FAIL
post_standard 1.9.0 1.10.0
...
MOCK_REMOTE_LOOKUP_FAIL=1; export MOCK_REMOTE_LOOKUP_FAIL
post_standard 1.9.0 1.10.0
...
releases v1.9.0
MOCK_PR_NUMBER=43; export MOCK_PR_NUMBER
run_script bash "$source_root/scripts/prepare-patch.sh"
assert_status 0
